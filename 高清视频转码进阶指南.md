...
如果只想在某些区域应用某些内容，则可以使用包装器rekt34或rekt_fast。后者仅将你的功能应用于给定的区域，从而加快了速度，对于抗锯齿和类似的慢滤波器非常有用。关于它的一些包装器已经有了，比如用于抗锯齿的rektaa。 rekt_fast和lambda函数相关，因此你需要输入rekt_fast(src, lambda x: core.f3kdb.Deband(x))，而非core.f3kdb.Deband(src)。
