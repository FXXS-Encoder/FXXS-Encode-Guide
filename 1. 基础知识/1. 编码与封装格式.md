...
注：flac-Free Lossless Audio
 Codec-自由无损音频压缩编码，flac也是常见的无损音频文件格式(flac用同样的名字做了容器名(即同时是封装格式的名字),<https://developer.mozilla.org/zh-TW/docs/Web/Media/Formats/Containers>)，而ape属于无损音频文件格式，但其并不代表编码方式。又比如m4a(封装格式,仅有音频的MPEG-4文件会使用.m4a扩展名)可以使用ALAC(Apple Lossless Audio Codec)编码也可以使用AAC(Advanced Audio Coding)编码。
