...
容器(Container)又名封装格式，常见的封装格式(MP4/MKV…)、而 H.264/H.265/AC3/AAC 属于编码方式(格式)，MP4以及MKV(封装格式)里封装的视频流以及音频流可以使用相同的编码格式
...
并且部分站点要求 Web-dl 标题必须写 H.265，不能使用HEVC，而原盘只允许写HEVC。（个人认为属于区域规则，不建议作为标准理解）
